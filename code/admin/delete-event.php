...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(['message' => 'Evénement supprimé avec succès']);
        } else {
            http_response_code(500);
            echo json_encode(['message' => 'Erreur lors de la suppression de l\'événement']);
        }
    } else {
        http_response_code(400);
        echo json_encode(['message' => 'ID d\'événement non fourni']);
    }
} else {
